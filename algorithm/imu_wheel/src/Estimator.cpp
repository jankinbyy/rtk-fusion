...
void Propagator::PropagateRK(const Eigen::Matrix3d &begin_G_R_I, const Eigen::Vector3d &gyro,
    ...
    dif_quarterion_f[0] = (-quaternion_self_last.x() * last_gyro[0] - quaternion_self_last.y() * last_gyro[1] - quaternion_self_last.z() * last_gyro[2]) * 0.5f;
    dif_quarterion_f[1] = (quaternion_self_last.w() * last_gyro[0] + quaternion_self_last.y() * last_gyro[2] - quaternion_self_last.z() * last_gyro[1]) * 0.5f;
    dif_quarterion_f[2] = (quaternion_self_last.w() * last_gyro[1] - quaternion_self_last.x() * last_gyro[2] + quaternion_self_last.z() * last_gyro[0]) * 0.5f;
    dif_quarterion_f[3] = (quaternion_self_last.w() * last_gyro[2] + quaternion_self_last.x() * last_gyro[1] - quaternion_self_last.y() * last_gyro[0]) * 0.5f;
    ...
    dif_quarterion_l[0] = (-med_quarterion[1] * gyro[0] - med_quarterion[2] * gyro[1] - med_quarterion[3] * gyro[2]) * 0.5f;
    dif_quarterion_l[1] = (med_quarterion[0] * gyro[0] + med_quarterion[2] * gyro[2] - med_quarterion[3] * gyro[1]) * 0.5f;
    dif_quarterion_l[2] = (med_quarterion[0] * gyro[1] - med_quarterion[1] * gyro[2] + med_quarterion[3] * gyro[0]) * 0.5f;
    dif_quarterion_l[3] = (med_quarterion[0] * gyro[2] + med_quarterion[1] * gyro[1] - med_quarterion[2] * gyro[0]) * 0.5f;
...
IMUData preIntegrate(const IMUData &imu_in, const std::shared_ptr<Estimator> &orientation_estimator) {
    ...
    const double timestamp = imu_in.timestamp;
    Eigen::Vector3d acc = imu_in.acc;
    Eigen::Vector3d gyro = imu_in.gyro;
    imuSelf.acc = acc;
    imuSelf.gyro = gyro;
    Status status = orientation_estimator->Estimate(timestamp, gyro, acc, &G_R_I);
    Eigen::Quaterniond quat(G_R_I);
    imuSelf.orientation = quat;
    return imuSelf;
}
...
void Update(const Eigen::Matrix3d &prior_G_R_I, const Eigen::Vector3d &prior_bg,
    ...
    Eigen::Vector3d residual =
        acc.normalized() - prior_G_R_I.transpose() * gravity_vec; // v=y-h(xt) imu加速度姿态校正，假设无机动加速度，只有重力加速度，y=am=an-R*g
    ...
    *posterior_cov = I_mins_KH * prior_cov * I_mins_KH.transpose() + K * acc_noise * K.transpose(); // p=(1-k*h)*p*(1-kh)t+kvkt P = (I-KH)P(I-KH)' + KRK'比P = (I-KH)P更稳定
}
